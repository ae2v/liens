...
export function QrCodesManager({ records, act }: { records: QrCodeRecord[]; act: AdminAction }) {
  ...
  return <><header className="admin-section-header"><div><h1>Codes QR</h1><p>Codes vectoriels, redirections et statistiques de scan.</p></div><button className="primary-button" onClick={() => setSelected("new")}><Plus />Créer un code</button></header>
    ...
    <div className="resource-list qr-resource-list">{visible.map((entry) => <article key={entry.id}><Image unoptimized width={86} height={86} src={`/api/qr?data=${encodeURIComponent(entry.trackingUrl)}&format=svg&dark=${encodeURIComponent(entry.foreground)}&light=${encodeURIComponent(entry.background)}`} alt="" /><button className="resource-copy" onClick={() => setSelected(entry.id)}><strong>{entry.name}</strong><span>{entry.targetUrl}</span><small>{entry.scans} scans · créé le {date(entry.createdAt)}</small></button><div className="resource-actions"><QrDownloads record={entry} /><button aria-label="Détails et statistiques" onClick={() => setSelected(entry.id)}><BarChart3 /></button></div></article>)}</div>
  </>;
}
...
function QrDetail({ record, act, onBack }: { record: QrCodeRecord; act: AdminAction; onBack: () => void }) {
  const [form, setForm] = useState(record); const [busy, setBusy] = useState(false);
  return <><button className="back-button" onClick={onBack}><ArrowLeft />Tous les QR codes</button><header className="admin-section-header"><div><h1>{record.name}</h1><p>{record.scans} scans · créé le {date(record.createdAt)}</p></div><QrDownloads record={{ ...form, trackingUrl: record.trackingUrl }} /></header><div className="detail-layout"><form className="detail-card edit-form" onSubmit={async (event) => { event.preventDefault(); setBusy(true); try { await act({ action: "updateQr", ...form }, "QR code enregistré"); } finally { setBusy(false); } }}><h2>Détails</h2><label>Nom<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>Destination<input value={form.targetUrl} onChange={(event) => setForm({ ...form, targetUrl: event.target.value })} /></label><div className="color-row"><label>Modules<input type="color" value={form.foreground} onChange={(event) => setForm({ ...form, foreground: event.target.value })} /></label><label>Fond<input type="color" value={form.background} onChange={(event) => setForm({ ...form, background: event.target.value })} /></label></div><button className="primary-button" disabled={busy}><Save />{busy ? "Enregistrement…" : "Enregistrer"}</button></form><section className="attached-qr"><Image unoptimized width={240} height={240} src={`/api/qr?data=${encodeURIComponent(record.trackingUrl)}&format=svg&dark=${encodeURIComponent(form.foreground)}&light=${encodeURIComponent(form.background)}`} alt={`QR code ${record.name}`} /><div className="inline-actions"><a className="secondary-button" href={`/api/qr?data=${encodeURIComponent(record.trackingUrl)}&format=svg&dark=${encodeURIComponent(form.foreground)}&light=${encodeURIComponent(form.background)}`} download><FileDown />SVG</a><a className="secondary-button" href={`/api/qr?data=${encodeURIComponent(record.trackingUrl)}&format=png&dark=${encodeURIComponent(form.foreground)}&light=${encodeURIComponent(form.background)}`} download><FileDown />PNG</a></div></section></div><AdminAnalytics id={record.id} kind="qr" /></>;
}
