...
export async function GET(request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  if (!link.enabled) return new NextResponse(html("Lien désactivé", link.disabledMessage, link.imageUrl ?? `${new URL(request.url).origin}/opengraph-image`, request.url, true), { status: 410, headers: { "Content-Type": "text/html; charset=utf-8" } });
  ...
  if (botPattern.test(userAgent)) {
    ...
    const image = meta.imageMode === "none" ? "" : meta.imageMode === "generated" ? `${origin}/api/share-image/${link.id}` : meta.imageMode === "upload" ? `${origin}/api/meta-image/${link.id}` : meta.imageUrl || `${origin}/opengraph-image`;
